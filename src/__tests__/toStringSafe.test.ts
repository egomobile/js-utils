...
describe("toStringSafe() function", () => {
    ...
    it.each([error, typeError])("should return string, if input is error object", (error) => {
        const expectedValue = `ERROR [${error.name}]: ${error.message}

${error.stack || ""}`;
        const str = toStringSafe(error);
